% Template for ICIP-2019 paper; to be used with:
%          spconf.sty  - ICASSP/ICIP LaTeX style file, and
%          IEEEbib.bst - IEEE bibliography style file.
% --------------------------------------------------------------------------

\documentclass{article}

\input{vAlgorithm}

\usepackage{spconf,amsmath,graphicx}
\usepackage{multicol, blindtext} % add here
\usepackage{booktabs}
\usepackage{xcolor}
%\usepackage[linesnumbered,boxed]{algorithm2e}
\newcommand\mycommfont[1]{\footnotesize\ttfamily\textcolor{blue}{#1}}
\SetCommentSty{mycommfont}

% Example definitions.
% --------------------
\def\x{{\mathbf x}}
\def\L{{\cal L}}

% Title.
% ------
\title{SYMBOLIC PROGRAM SLICING ON SMART CONTRACTS}
%
% Single address.
% ---------------

\name{Zi Xuan Chen, Fang Yu}
\address{National Chengchi University \\
Department of Computer Science, Management Information Systems \\
104703010@nccu.edu.tw, yuf@nccu.edu.tw}

%
% For example:
% ------------
%\address{School\\
%	Department\\
%	Address}
%
% Two addresses (uncomment and modify for two-address case).
% ----------------------------------------------------------
%\twoauthors
%  {Zi Xuan Chen
%    %\sthanks{The fourth author performed the work while at ...}
%  }
%	{National Chengchi University, \\ Department of Computer Science \\
%	s10410@nccu.edu.tw}
%  {Fang Yu
%    %\sthanks{Thanks to XYZ agency for funding.}
%  }
%	{National Chengchi University, \\ Department of Management Information Systems, \\
%	yuf@nccu.edu.tw}


\begin{document}
%\ninept
%

\maketitle

%

\begin{abstract}

  %\input{abstract.tex}
  we propose a method to do program slicing on stack-based programming language. With slicing, we can analyse properties more efficiently. We take the EVM bytecode\cite{wood2014ethereum} as the example language, which is used to write smart contract\cite{szabo2003advances} on Ethereum\cite{wood2014ethereum}.

\end{abstract}

\begin{keywords}
  BlockChain, Ethereum, Slicing, Verification
\end{keywords}


\section{Introduction}
\label{sec:introduction}

\section{Related Work}
\label{sec:relatedwork}

\section{Method}
\label{sec:method}

\RestyleAlgo{algoruled} % ruled algorithm
\begin{vAlgorithm}[ht!]{0.56\textwidth}{1em}
  \SetNoFillComment
  %{algorithm width}{indent width}
  \caption{AnalysisEnvironment}
  \KwIn{$StackDG$}

  \tcc{Environment variables}

  evaled = $\O$\;
  conAddrs = dictionary()\;
  addrDepPCs = dictionary()\;
  conOffsets = dictionary()\;
  offsetDepPCs = dictionary()\;
  conVals = dictionary()\;
  valDepPCs = dictionary()\;
  rwDependency = dictionary()\;

  \tcc{Return All dependant program counters}

  \Def{depPCs(inst)}{
    \Return addrDepPCs[inst.pc] \\ \quad \quad \quad $\cup$ offsetDepPCs[inst.pc] \\ \quad \quad \quad $\cup$ valDepPCs[inst.pc]
  }

  \tcc{check insts if have same addr parameters }

  \Def{addrOverlap(insA, insB)}{
    rangeA = product(conAddrs[insA.pc], \\ \quad \quad \quad \quad \quad \quad \quad \ \ conOffsets[insA.pc])\;
    rangeB = product(conAddrs[insB.pc], \\ \quad \quad \quad \quad \quad \quad \quad \ \ conOffsets[insB.pc])\;
    \For{(Aa, Ao), (Ba, Bo) $\in$ product(rangeA, rangeB)}{
      \uIf{\{Aa..Aa + Ao\} $\cap$ \{Ba..Ba + Bo\} $\neq$ $\O$}{
        \Return True\;
      }
    }
    \Return False\;
  }

  \tcc{Initialize Environment}

  \Def{initialize()}{

    \Global $Environment$ $variables$\;
    readIns = StackDG.readIns \;
    writeIns = StackDG.writeIns \;

    \tcc{eval instructions' parameters (addr)}
    addrs = [ i.pc, eval(i.addrs) $|$ i $\in$ readIns $\cup$ writeIns ] \;

    conAddrs = \{ pc: con $|$ (pc, (con, \_)) $\in$ addrs \}\;
    addrDepPCs = \{ pc: dep $|$ (pc, (\_ , dep)) $\in$ addrs \}\;

    \tcc{eval instructions' parameters (offset)}
    offsets = [ i.pc, eval(i.offsets) $|$ i $\in$ readIns $\cup$ writeIns ] \;

    conOffsets = \{ pc: con $|$ (pc, (con, \_)) $\in$ offsets \}\;
    offsetDepPCs = \{ pc: dep $|$ (pc, (\_ , dep)) $\in$ offsets \}\;

    \tcc{eval instructions' parameters (val)}
    vals = [ i.pc, eval(i.vals) $|$ i $\in$ writeIns ] \;
    conVals = \{ pc: con $|$ (pc, (con, \_)) $\in$ vals \}\;
    valDepPCs = \{ pc: dep $|$ (pc, (\_ , dep)) $\in$ vals \}\;

    \tcc{write insts that can't be re-evaled}
    evaled = \{pc $\in$ addrDepPCs $|$ addrDepPCs[pc] = $\O$\} \\
    \quad $\cap$ \{pc $\in$ offsetDepPCs $|$ offsetDepPCs[pc] = $\O$\} \\
    \quad $\cap$ \{pc $\in$ valDepPCs $|$ valDepPCs[pc] = $\O$\}
  }

\end{vAlgorithm}

\RestyleAlgo{algoruled} % ruled algorithm
\begin{vAlgorithm}[ht!]{0.57\textwidth}{1em}
  %{algorithm width}{indent width}
  \SetNoFillComment
  \caption{buildAddressDependency}
  \KwIn{$CFG$, $StackDG$}
  \KwOut{$AddressDG$}

  \Import AnalysisEnvironment \As env

  \Def{buildAddressDependency()}{
    \tcc{initialize the environment}
    env.initialize()\;
    \tcc{declare and alias variables}
    visit, alter = $\O$, $\O$ \;
    sreads = StackDG.SLOADs \;
    swrites = StackDG.SSTOREs \;
    mreads = StackDG.MSTOREs \;
    mwrites = StackDG.\{MLOAD $\cup$ SHA3 \\ \quad \quad \quad \quad \quad $\cup$ CREATE $\cup$ CALL $\cup$ RETURN\}s

    \tcc{build addr dependency of \\ storage and memroy}
    \While{True}{
      evaled = env.evaled.copy()\;
      buildDepend(swrites, sreads, visit)\;
      buildDepend(mwrites, mreads, visit)\;
      \uIf{exist write inst can be re-evaled}{
        \For{inst $\in$ re-evaled}{
          update $Environment$ $variables$ in env \\ \quad \quad with eval($instruction$ $parameters$)
        }
      }
      \uIf{env.evaled $\setminus$ evaled = $\O$}{
        \Break\;
      }
    }
    \Return env.rwDependency\;
  }

  \tcc{helper function}
  \Def{buildDepend(writes, reads, visit)}{

    concrete = \{ins $\in$ (writes $\setminus$ visit) \\ \quad \quad \quad \quad \quad \quad \ \ $|$ \ \ $env.depPCs(ins)$ = $\O$\}\;
    \While{concrete $\neq$ $\O$}{

      \For{inst $\in$ concrete}{
        block = CFG.blockOf(inst)\;
        dfsCFG(inst, block, writes, reads, $\O$)\;
      }
      visit.update(concrete)\;
      \For{inst $\in$ (writes $\setminus$ visit)}{
        \uIf{(env.depPCs(inst) $\setminus$ env.evaled) = $\O$}{
          update $Environment$ $variables$ in env \\ \quad \quad with eval($instruction$ $parameters$)
        }
      }

      concrete = \{ins $\in$ (writes $\setminus$ visit) \\ \quad \quad \quad \quad \quad \quad \ \ $|$ \ \ $env.depPCs(ins)$ = $\O$\}\;
    }
  }
\end{vAlgorithm}

\RestyleAlgo{algoruled} % ruled algorithm
\begin{vAlgorithm}[ht!]{0.57\textwidth}{1em}
  %{algorithm width}{indent width}
  \SetNoFillComment
  \caption{dfsCFG}
  \KwIn{$CFG$, $StackDG$}
  \KwOut{$AddressDG$}

  \Import AnalysisEnvironment \As env

  \tcc{do CFG dfs for building dependency}
  \Def{dfsCFG(wInst, block, writes, reads, visit)}{
    visit.add(block)\;
    rwInsts = (block.insts $\cap$ writes $\cap$ reads)\;
    \uIf{wInst $\in$ block}{
      rwInsts = rwInsts $\setminus$ \\ \quad \quad \{ ins $\in$ block.insts $|$ ins.pc $>$ wInst.pc\}\;
    }
    \For{inst $\in$ rwInsts}{
      \tcc{if "exist the probability" to re-write the same address then return, "probability" means the "or" part}
      \uIf{inst.name = wInst.name
        \AndL (env.addrOverlap(wInst, inst) \\ \quad \quad \OrL env.addrDepPCs[inst.pc] $\neq$ $\O$)}{
        visit.remove(block)\;
        \Return;
      }
      \uIf{inst $\in$ reads}{
        depPCs = env.addrDepPCs[inst.pc] $\cup$ \\ \quad \quad \quad \quad \ env.offsetDepPCs[inst.pc]\;
        \uIf{depPCs $\neq$ $\O$ \AndL \\
          \quad  depPCs $\setminus$ env.evaled = $\O$}{
          update $Environment$ $variables$ in env \\ \quad \quad with eval($instruction$ $parameters$)
        }

        \uIf{addrOverlap(wInst, inst)}{
          env.evaled.add(pc)\;
          env.rwDependency[inst.pc].add(wInst)\;
          env.conVals[inst.pc].update(\\
          \quad \quad \quad \quad \quad \quad \ \ env.conVals[wInst.pc]);
        }
      }
    }
    \For{nextBlock $\in$ block.outgoingBlock}{
      dfsCFG(wInst, nextBlock, writes, reads, visit)\;
    }
    visit.remove(block)\;
  }

\end{vAlgorithm}

\RestyleAlgo{algoruled} % ruled algorithm
\begin{vAlgorithm}[ht!]{0.55\textwidth}{1em}
  %{algorithm width}{indent width}
  \SetNoFillComment
  \caption{eval}
  \KwIn{$StackDG$, instruction, visit}
  \KwOut{$concrete$ $values$, $dependant$ $PCs$}

  \Def{eval(inst, visit)}{
    \uIf{inst $\in$ visit}{
      \Return \{\}, \{inst\}\;
    }
    visit.add(inst)\;
    concrete, dependant = $\O$, $\O$\;
    \uIf{inst.name.startswith('PUSH')}{
      \Return \{int(op.operand)\}, \{\}\;
    }
    cons, deps = \{map(eval(\_ , visit), argList) \\ \quad \quad \quad \quad \quad \quad \quad \quad $|$ argList $\in$ inst.argLists\}$^T$\;
    \For{argList $\in$ cons}{
      val = None\;
      \uIf{None $\in$ argList}{\Continue\;}
      \uElseIf{inst.name = 'ADD'}{
        val = \Let x, y = argList \In x + y\;
      }
      \uElseIf{inst.name = 'SUB'}{
        val = \Let x, y = argList \In x - y\;
      }
      \uElseIf{inst.name = 'MUL'}{
        val = \Let x, y = argList \In x * y\;
      }
      \uElseIf{inst.name = 'DIV'}{
        val = \Let x, y = argList \In x / y\;
      }
      \uElseIf{inst.name = 'EXP'}{
        val = \Let x, y = argList \In $x^{y}$\;
      }
      \uElseIf{inst.name = 'ISZERO'}{
        \[
        val = \Let [x] = argList \ \In
          \begin{cases}
            0,& \text{if } x = 0\\
            1,              & \text{otherwise}
          \end{cases}\;
        \]
      }
      \uElseIf{inst.name = 'NOT'}{
        val = \Let [x] = argList \In (1 $<<$ 256) - 1 - x\;
      }
      \uElseIf{inst.name = 'AND'}{
        val = \Let x, y = argList \In $x \ \& \ y$\;
      }
      \uElseIf{inst.name = 'OR'}{
        val = \Let x, y = argList \In $x \ | \ y$\;
      }
      \uElseIf{inst.name = 'EQ'}{
        val = \Let x, y = argList \In $x \ = \ y$\;
      }
      \uElseIf{inst.name $\in$ \{'MLOAD', 'SLOAD', 'SHA3'\}}{
        concrete.update(env.conVals[inst.pc])\;
      }
      \uElse{
        \tcc{SHA3 not impl yet}
        trhow Excption("not handle the inst yet")\;
      }
      \uIf{val $\neq$ None}{ concrete.add(val)\; }
      dependant.update(concat(deps))\;
    }
    visit.remove(inst)\;
    \Return concrete, dependant\;
  }

\end{vAlgorithm}

\section{experiment}
\label{sec:experiment}

\section{Conclusion}
\label{sec:conclusion}

% To start a new column (but not a new page) and help balance the last-page
% column length use \vfill\pagebreak.
% -------------------------------------------------------------------------
%\vfill
%\pagebreak

% References should be produced using the bibtex program from suitable
% BiBTeX files (here: strings, refs, manuals). The IEEEbib.bst bibliography
% style file from IEEE produces unsorted bibliography list.
% -------------------------------------------------------------------------
\bibliographystyle{IEEEbib}
\bibliography{strings,refs}

\end{document}
